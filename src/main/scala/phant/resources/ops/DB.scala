...
object db {
  ...
  object Splitter {
    ...
    implicit def SuccSplitter[N <: Nat, Db <: DB](implicit
  ...
  object Withdrawer {
    ...
    implicit def SuccWithdrawer[N <: Nat, Db <: DB](implicit
  ...
  object Injector {
    ...
    implicit def SuccInject[N <: Nat, Db <: DB, Col](implicit
                                                     ijtor: Injector[N,Db#Tail,Col]) =
      new Injector[Succ[N],Db,Col] {
        type Out = |:[Db#Head, ijtor.Out]
        def inject(db: Db, col: Seq[Col]) = |:(db.head, ijtor.inject(db.tail, col))
      }
  }
