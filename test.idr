...
places : Guard (Plain [D,N,A]) (Plain [D,N,A]) (Expr (SCH [A]))
...
meetings : Guard (Plain [D,N,A]) (Plain [D,N,A]) (Expr (SCH [D,Count]))
...
compose : Guard (Plain [D,N,A]) (Plain [D,N,A]) (Expr (SCH [A,D,Count]))
compose = places >>= \q1 =>
          meetings >>= \q2 =>
          Pure (ExprProduct q1 q2)

composeDo : Guard (Plain [D,N,A]) (Plain [D,N,A]) (Expr (SCH [A,D,Count]))
composeDo = do
  q1 <- places
  q2 <- meetings
  pure (ExprProduct q1 q2)
...
placesF : Guard (Plain [D,N,A]) (FragV [[D,Id], [Nc,A,Id]]) (Expr (SCH [A]))
...
placesFDo : Guard (Plain [D,N,A]) (FragV [[D,Id], [Nc,A,Id]]) (Expr (SCH [A]))
placesFDo = do
  Encrypt "mykey" N
  Frag [[D]]
...
placesF' : Guard (Plain [D,N,A]) (FragV [[D,Id], [Nc,A,Id]]) (Expr (SCH [D,A]))
placesF' = Encrypt "mykey" N                                      >>= \_ =>
           Frag [[D]]                                             >>= \_ =>
           QueryF 0 (Project [D, Id] . Select D nextWeek)         >>= \dIds =>
           Let (MkVar "ids" (ExprProject [Id] dIds)) (                \ids =>
           QueryF 1 (Project [A] . Select Id (flip ExprElem ids)) >>= \res =>
...
placesFDo' =  do
  ...
  Let (MkVar "ids" (ExprProject [Id] dIds)) (\ids => do
  -- let ids = ExprProject [Id] dIds
    res <- QueryF 1 (Project [A] . Select Id (flip ExprElem ids))
    pure (ExprProject [D,A] $ ExprProduct dIds res))
